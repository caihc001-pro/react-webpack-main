...
export function handleError(event) {
  ...
  if (lastErrorTime && now - lastErrorTime < 1000) {
    // 1秒内的重复错误将被忽略
    return;
  }
...
function parseError(input) {
  ...
  const eventHandlers = {
    error: (event) => {
      ...
      return {
        ...
        position:
          stackInfo.position ||
          (event.lineno && event.colno ? `${event.lineno}:${event.colno}` : ""),
      };
    },
    unhandledrejection: (event) => {
      ...
      return {
        ...
        stack:
          event.reason instanceof Error ? getLines(event.reason.stack) : "",
  ...
  if (
    ...
    input.target &&
    (input.target.src || input.target.href)
  ) {
...
function parseStack(stack) {
  ...
  const match = stack.match(
    /((?:https?:\/\/|file:\/\/|webpack-internal:\/\/\/).*?):(\d+):(\d+)/
  );
  if (match) {
    return {
      filename: match[1],
